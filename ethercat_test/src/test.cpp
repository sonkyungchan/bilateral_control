...
void EPOS_CSP(void *arg)
{
    ...
    while (run)
    {
        ...
        if (sys_ready)
        {
            ...
            float T_d1 = teletorque1/1.0;
            float T_d2 = teletorque2/1.0;
//	    printf("%f\n", T_d);
            ...
            ival[1] = radtopulse(v2);
            for (i=0; i<NUMOFEPOS4_DRIVE; ++i)
            {
                if (ival[i] >= 50000) ival[i] = 50000;
                if (ival[i] <= -50000) ival[i] = -50000;
            }
            ival[0] = 0;
            ival[1] = 0;
            ...
            {
                if (i%2==0)
                    epos4_drive_pt[i].ptOutParam->TargetPosition=ival[i]/4 + zeropos[i];
                else
