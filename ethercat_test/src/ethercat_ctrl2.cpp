...
if (gt>1.0) k_d = 0.9*k_eff;
//if (gt>1.0+10.0) k_d = 0.5*k_eff;
//if (gt>1.0+20.0) k_d = 0.1*k_eff;
if (gt>1.0+10.0) k_d = 0.01*k_eff;
//if (gt>1.0+30.0) k_d = 0.0001*k_eff;


float q_ed = 0;
float dq_ed = 0;
	    dq_e=(q_e-q_e_)/(dt*0.000000001);
	    //Torque_desired : T_d, q_d desried : q_dd, error : e = q_dd-q_d, error time derivative :E
	    T_d =k_d*(q_ed-q_e)+b_d*(dq_ed - dq_e);
	    q_dd = k_eff*q_d;
//	    e_ = q_dd-q_d_;
	    q_e_ =q_e;
	    e = T_d-q_dd;
	    ...
	    P_gain = 2;
	    D_gain = 5;
	    Con = P_gain*e+D_gain*E;
	    reference = (28/25)*((T_d-k_eff*q_d)/k_eff+q_p);
//	    reference = (28/25)*Con;
//	    reference = (28/25)*(Con+1)/Con*q_d;
	    rt_printf("%0.3f\n",k_d);
