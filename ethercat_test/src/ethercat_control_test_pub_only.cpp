...
void pub_run(void *arg)
{
    ...
    ros::Publisher pub = n.advertise<ethercat_test::pos>("measure",1);
    ...
    while (run)
    {
        ...
        if (inOP==TRUE)
        {
            ...
            else
            {
                ...
                msg.position[0] = epos4_drive_pt[0].ptInParam -> VelocityActualValue;
             //   msg.position[1] = epos4_drive_pt[1].ptInParam -> VelocityActualValue;
            //    msg.position[2] = epos4_drive_pt[2].ptInParam -> VelocityActualValue;
pub.publish(msg);
