...
*/
	    ...
	    if (gt > 1.0) reference = 0.05;
/*	    if (gt > 1.0+T_end) reference = -0.05;
	    if (gt > 1.0+2*T_end) reference = 0.05;
	    if (gt > 1.0+3*T_end) reference = -0.05;
	    if (gt > 1.0+4*T_end) reference = 0.1;
	    if (gt > 1.0+5*T_end) reference = -0.1;
	    if (gt > 1.0+6*T_end) reference = 0.1;
	    if (gt > 1.0+7*T_end) reference = -0.1;
	    if (gt > 1.0+8*T_end) reference = 0.15;
	    if (gt > 1.0+9*T_end) reference = -0.15;
	    if (gt > 1.0+10*T_end) reference = 0.15;
	    if (gt > 1.0+11*T_end) reference = -0.15;
	    if (gt > 1.0+12*T_end) reference = 0.2;
	    if (gt > 1.0+13*T_end) reference = -0.2;
	    if (gt > 1.0+14*T_end) reference = 0.2;
	    if (gt > 1.0+15*T_end) reference = -0.2;
	    if (gt > 1.0+16*T_end) reference = 0.25;
	    if (gt > 1.0+17*T_end) reference = -0.25;
	    if (gt > 1.0+18*T_end) reference = 0.25;
	    if (gt > 1.0+19*T_end) reference = -0.25;
	    if (gt > 1.0+20*T_end) reference = 0.3;
	    if (gt > 1.0+21*T_end) reference = -0.3;
	    if (gt > 1.0+22*T_end) reference = 0.3;
	    if (gt > 1.0+23*T_end) reference = -0.3;
*/	    if (gt > 1.0+24*T_end) reference = 0;
...
//	    if (teletorque < thres) reference = (0-q_d)*2;
//            if (teletorque >= thres) reference = (teletorque-q_d);
//	    v = reference+q_p*r_j/r_m;
/*	    tq_d = 0.1;
	    tq = k_eff*q_d;
	    kp = 5;
	    kd = 3;
//	    reference = r_j/r_m*(-(-k_d+k_eff)/k_d*q_d);

	    err = tq_d - tq;
	    derr = err-err_;
	    err_ = err;
	    reference = kp*err + kd*derr;
*/	    v = reference;
            ival = radtopulse(v);
//	    printf("%f %d\n",v, ival);
